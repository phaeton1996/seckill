...
public class VerifyCodeController {
    ...
    @ResponseBody
    public Result<CodeMsg> checkVerifyCode(HttpServletRequest request) {
        if (!VerifyCodeUtil.checkVerifyCode(request)) {
            return Result.returnWithCodeMsg(USER_CODE_LEGAL);
        } else {
            return Result.returnWithCodeMsg(USER_CODE_ILLEGAL);
        }
    }
}
